 ...
 */
public class QLearningAlphaDecay extends LearningAlgorithm{
	...
	public void learnPolicy(){
		...
		for (int nIteration=0; nIteration<iterations; nIteration++){
			...
			double Q, reward, maxQ;               // Values necessary to update the table.
                        alpha=alpha/(nIteration+1);//CHECK A DECAY LESS EFFECTIVE TO IMPROVE DATA alpha=alpha^2/(nIteration+1) or sth like that
			...
			if (args.length>1){
				try{
					...
					System.out.println("The value for the number of iterations is not correct. Using 1000.");
                            ...
                            if (args.length>2){
				try{
					probGreedy = Integer.parseInt(args[2]);
				} 
				catch(Exception e){
					System.out.println("The value for the probability of greedy is not correct. Using 0.9");
				}		
                            }    
			}
		}
